...
* Date    : novembre-décembre 2011 / février & mai 2016                        *
...
* Mais comme on n'a que 5 bank libres sur MO6 et que la boucle sonore occupe
*    quasiment 6 bank, on doit réduire la fréquence à 12085 Hz (14187 sur TO8)
*    pour faire 5 bank de 16357 octets. Ce qui fait une alternance de 24 cycles
*    pour l'anim puis 54 cycles pour le sample (652625/54=12085).
*
...
CHMT_BANK2
      ...
      STA $A7E5       * commute la nouvelle bank
      LDU #SAMPLE
      PULS D
      JMP LECTURE
LECTURE_S
...
FRAME_ANIM
   ...
   STA FRAME_ANIM_YVAL+1    * modifie le CMPX de FRAME_ANIM_YVAL
   STB FRAME_ANIM_YVAL+2    * modifie le CMPX de FRAME_ANIM_YVAL
      JSR LECTURE     * 24 cycles
...
FRAME_ANIM_B
   ...
   TFR D,X         * X contient les donnees graphiques
      JSR LECTURE     * 23 cycles (au lieu de 24 cycles)
   
* on a la disposition suivante sur MO6 :
* RAMA $C000-$CFFF (haut de l'écran) puis $D000-$DFFF (bas de l'écran)
* RAMB $E000-$EFFF (haut de l'écran) puis $B000-$BFFF (bas de l'écran)
* ce qui impose de complexifier l'algorithme de dessin pour gérer
* le saut de page en RAMB, qui tombe au milieu d'une ligne :?
* Afin de bien synchroniser la sortie du sample, il faut que le nombre
* de cycles soit identique quelle que soit la position en ram video,
* cela rend donc difficile l'utilisation de CMPY / BHS / BRA.
* Le plus simple est de remonter le graphisme pour ne pas avoir a changer
* de page en milieu de routine. On ajoute donc les instructions ci-dessous
* pour modifier le code en fonction de l'adresse video du début du dessin.
* La routine remplace une serie d'intructions vides sur TO8, placées pour
* synchroniser le son. Il y a un décalage d'un ou deux cycles sur MO6 mais
* c'est uniquement à chaque début de dessin.
   TFR Y,D         * D (A et B) contient l'adresse video
   ROLA
   ROLA
   ANDA #$40
   EORA #$40
   ORA #$A0        * devient $A0 si Y >= $1000, $E0 sinon
   STA DESSIN_MODIF_MO6+2
   STA DESSIN_MODIF_MO6+6

      JSR LECTURE     * 26 cycles (au lieu de 24 cycles)
      NOP
...
DESSIN_B
      LEAX 0,X        * 5 cycles
DESSIN_B2
   STA $C000,Y     * ecrit les 2 points contenus dans A en RAMA
   STA $C028,Y     * ecrit les 2 points contenus dans A sur la ligne suivante en RAMA
      JSR LECTURE     * 24 cycles depuis DESSIN / 24 cycles depuis BNE DESSIN_B
*   CMPY #$1000
*   BHS DESSIN_MO6_HACK
DESSIN_MODIF_MO6
   STB $E000,Y     * ecrit les 2 points contenus dans B en RAMB
   STB $E028,Y     * ecrit les 2 points contenus dans B sur la ligne suivante en RAMB
*   BRA DESSIN_MO6_HACK_FIN
*DESSIN_MO6_HACK
*   * Y est > $1000, on écrit donc en $B000
*   STB $A000,Y     * ecrit les 2 points contenus dans B en RAMB
*   STB $A028,Y     * ecrit les 2 points contenus dans B sur la ligne suivante en RAMB
*DESSIN_MO6_HACK_FIN
